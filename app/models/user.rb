...
class User < ActiveRecord::Base
  ...
  def self.sendSignUpReminderForCurrentMonth(users)
    events =  Event.eventsForMonth(Time.now.month, Time.now.year) 
    users.each do |user|
      UserMailer.currentMonthSignupReminder(user,events).deliver
    end
  end
end
